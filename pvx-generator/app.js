...
Promise.all([
    ...
    // Support both string arrays and object arrays for future metadata upgrades
    const allSkillNames = Object.values(skillsData)
        .map(s => typeof s === 'object' ? s.name : s)
        .filter(s => s && s !== "No Skill")
        .sort();
...
document.getElementById('reset-btn').addEventListener('click', () => {
    if(confirm("Are you sure you want to clear the entire build?")) {
        ...
        document.getElementById('build-usage').value = "";
        document.getElementById('build-counters').value = "";
        document.getElementById('build-notes').value = "";
        document.getElementById('build-see-also').value = "";
...
document.getElementById('generate-btn').addEventListener('click', () => {
    ...
    const makeLink = (str, displayStr) => {
        if (!str) return '';
        let display = displayStr || str;
        if (str === 'Martial Weapon') return `[[gww:Martial weapon|${display}]]`;
        return `[[gww:${str}|${display}]]`;
    };
    ...
    wiki += `== Attributes and Skills ==\n<pvxbig>\n`;

    let finalAttributes = {};
    document.querySelectorAll('.manual-attr-input').forEach(inp => {
        let pts = parseInt(inp.value) || 0;
        if(pts > 0) finalAttributes[inp.dataset.attr] = { base: pts, rune: 0, head: 0 };
    });
    ...
    armorDivs.forEach(div => {
        ...
        if (headSelect && headSelect.value) {
            ...
            if (!finalAttributes[attr]) finalAttributes[attr] = { base: 0, rune: 0, head: 0 };
            finalAttributes[attr].head = 1;
        }

        const rawRune = div.querySelector('.rune-select').value.replace(/ \([^)]+\)/g, '');
        if (rawRune) {
    ...
    // Build the PvX [build] tag
    let secProf = currentSecondary === "None" ? "any" : currentSecondary;
    let buildTag = `[build prof=${currentProfession}/${secProf}`;

    // Grab the name and sanitize double quotes
    const buildName = document.getElementById('build-name').value.trim();
    if (buildName) {
        let safeName = buildName.replace(/"/g, "'"); 
        buildTag += ` name="${safeName}"`;
    }

    for (let attr in finalAttributes) {
        let data = finalAttributes[attr];
        if (data.base > 0 || data.rune > 0 || data.head > 0) {
            let attrName = attr.replace(/ /g, '');
            let valStr = `${data.base}`;
            if (data.head > 0) valStr += `+${data.head}`;
            if (data.rune > 0) valStr += `+${data.rune}`;
            buildTag += ` ${attrName}=${valStr}`;
        }
    }
    buildTag += `]`;
    ...
    optRows.forEach(row => {
        ...
        if (name) {
            ...
            // Future-proofing: If skillsData is updated to contain objects with an "attribute" property
            const skillObj = Object.values(skillsData).find(s => s && typeof s === 'object' && s.name === name);
            if (skillObj && skillObj.attribute && finalAttributes[skillObj.attribute]) {
                const fa = finalAttributes[skillObj.attribute];
                const totalAttr = fa.base + fa.rune + fa.head;
                attrLvl = `|${totalAttr}`;
            }

            let line = `* {{skill pvxicon|${name}${attrLvl}}}`;
            if (desc) {
                line += ` ${desc}`;
            }
            optSkillsMd += `${line}\n`;
    ...
    function stripDesc(str) { return str ? str.replace(/ \([^)]+\)/g, '') : ''; }
        ...
        if (runeCount > 0) {
            if (r) {
                if (r.startsWith("Rune of Vigor (minor, major, or superior)")) {
                    runes.push(`[[gww:Rune of Vigor|Vigor (minor, major, or superior)]]`);
                } else {
                    let displayR = r.replace(/^Rune of /, '');
                    runes.push(makeLink(r, displayR));
                }
            } else {
                runes.push(`[[gww:Optional|Optional]]`);
            }
        }

        if (insigniaCount > 0) {
            if (i) {
                let displayI = i.replace(/ Insignia$/, '');
                insignias.push(makeLink(i, displayI));
            } else {
                insignias.push(`[[gww:Optional|Optional]]`);
            }
        }
    });
    ...
    if (runes.length > 0) wiki += `'''Runes'''\n* ${runes.join(' | ')}\n\n`;
    if (insignias.length > 0) wiki += `'''Insignias'''\n* ${insignias.join(' | ')}\n\n`;
    ...
    if (validWeaponSets.length === 1) wiki += `'''Weapon set'''\n* ${validWeaponSets[0]}\n\n`;
    else if (validWeaponSets.length > 1) {
        wiki += `'''Weapon sets'''\n`;
        validWeaponSets.forEach((ws) => wiki += `* ${ws}\n`);
        wiki += `\n`;
    }

    // Usage
    const buildUsage = document.getElementById('build-usage').value.trim();
    if (buildUsage) {
        wiki += `== Usage ==\n${buildUsage}\n\n`;
    }

    // Counters
    const buildCounters = document.getElementById('build-counters').value.trim();
    if (buildCounters) {
        wiki += `== Counters ==\n${buildCounters}\n\n`;
    }

    // Notes
    const buildNotes = document.getElementById('build-notes').value.trim();
    if (buildNotes) {
        wiki += `== Notes ==\n${buildNotes}\n\n`;
    }

    // See Also
    const buildSeeAlso = document.getElementById('build-see-also').value.trim();
    if (buildSeeAlso) {
        wiki += `== See also ==\n${buildSeeAlso}\n\n`;
    }

    wiki += `----\n[https://guild-wars-tools.github.io/pvx-generator/ PvXwiki Build Generator]`;

    const outputString = wiki.trim();
